...
for (let i = 1; i <= 62; i++) {
  ...
  if (lastDivIndex === -1) continue;
  
  let theDiv = contentBeforeMainEnd.substring(lastDivIndex);
  if (!theDiv.includes('Return to Course')) {
     console.log('Skipping ' + file + ' - last div does not contain Return to Course');
     continue;
  }
